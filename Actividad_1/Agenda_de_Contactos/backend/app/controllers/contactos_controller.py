...
def crear_contacto(datos: ContactoCreate) -> Contacto:
    ...
    nuevo_contacto = Contacto(
        ...
        correo=datos.correo,
        categoria=datos.categoria #! Aqui se cambio
    )
...
def buscar_contactos(texto: str) -> list[Contacto]: #! Se modifico esto
    """
    ...
    resultados = [
        ...
        or texto_normalizado in contacto.categoria.lower() #! Se modifico aqui
    ]

    return resultados

def obtener_contactos_por_categoria(contactos_categoria: str) -> list[Contacto]: #! Aqui se agrego esto
    """
    Devuelve la lista de contactos por categoria.

    Esta función representa la lógica de negocio asociada
    a la consulta del contacto.

    Retorna:
    - una lista de objetos Contacto por categorias
    """
    Categorias_Validas = ['Familia','Trabajo','Personal']
    categoria_normalizada = contactos_categoria.strip().lower()

    if contactos_categoria not in Categorias_Validas:
        raise HTTPException(
        status_code=400,
        detail="Categoria NO valida"
    )
    resultados = [
        contacto 
        for contacto in contactos_db 
        if contacto.categoria.lower() == categoria_normalizada
    ]
